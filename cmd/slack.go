...
func handleSlackAuthRequest(c echo.Context) error {
	...
	if len(code) > 0 {
		...
		if len(jsonBody.AccessToken) == 0 {
			...
			return c.Redirect(307, app.ko.String("server.ui")+"/integrate-slack?ser="+slackErrBody.Error)
		} else {
			qp := url.Values{}
			qp.Set("sat", jsonBody.AccessToken)
			qp.Set("u", jsonBody.AuthedUser.Id)
			return c.Redirect(307, app.ko.String("server.ui")+"/integrate-slack?"+qp.Encode())
		}
	}

	return c.Redirect(307, app.ko.String("server.ui")+"/integrate-slack?ser=code_not_found")
}
